...
- **Table-of-contents links were corrupted in books whose package document isn't at the archive root.** Rebuilding the toc wrote full archive paths (`OEBPS/text/chapter-1.xhtml`) into a navigation document whose links are relative to the package document's own directory (`text/chapter-1.xhtml`), producing dead links. For the same reason, several `validate_epub` checks (`toc-spine-order`, `toc-label-heading-mismatch`, `ncx-toc-divergence`, `dangling-href`) reported false positives on such books. Both now resolve hrefs before comparing.
